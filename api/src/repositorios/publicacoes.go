...
func (repositorioPubli Publicacoes) BuscarPorID(publicacaoID uint64) (modelos.Publicacao, error) {
	linha, erro := repositorioPubli.db.Query(`
	...
	if linha.Next() {
		if erro = linha.Scan( //A ordem deve ser do select, da tabela
			&publicacao.ID,
			&publicacao.Titulo,
			&publicacao.Conteudo,
			&publicacao.AutorID,
...
// Atualizar altera os dados de uma publicação no banco, só retorna um error
...
// Deletar excluir uma publicação do banco de dados
...
func (repositorio Publicacoes) BuscarPorUsuario(usuarioID uint64) ([]modelos.Publicacao, error) {
	...
	// crio um slice de publicações
	var publicacoes []modelos.Publicacao

	for linhas.Next() {
		var publicacao modelos.Publicacao
		if erro = linhas.Scan( //A ordem deve ser do select, da tabela
			&publicacao.ID,
			&publicacao.Titulo,
			&publicacao.Conteudo,
			&publicacao.AutorID,
