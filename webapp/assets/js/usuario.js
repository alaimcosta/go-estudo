...
function pararDeSeguir() {
    ...
    $.ajax({
        ...
        window.location = `/usuarios/${usuarioId}`; //somente recarrega a página
    }).fail(function() {
...
function atualizarSenha(evento) {
    ...
    $.ajax({
        ...
        Swal.fire("Ops...", "Erro ao atualizar a senha do usuário no banco!", "error");
    });
}
...
function deletarUsuario() {
    Swal.fire({ //para acessar 
        title: "Atenção!",
        text: "Tem certeza que deseja apagar a sua conta? Essa é uma ação irreversível!",
        showCancelButton: true, //mostrar a opção de cancelar a ação para o usuário
        ...
        if (confirmacao.value) {
            $.ajax({
                ...
                Swal.fire("Sucesso!", "Seu usuário foi excluído com sucesso!", "success")
                    .then(function() {
                        window.location = "/logout"; //esperar o usuário clicar no botão OK para assim ser redirecionado para a página de logout
                    })
            }).fail(function() {
