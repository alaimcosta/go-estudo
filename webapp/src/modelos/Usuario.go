...
// Usuario representa uma pessoa usando a rede social/aplicação
type Usuario struct {
	ID          uint64       `json:"id"`
...
func BuscarUsuarioCompleto(usuarioID uint64, r *http.Request) (Usuario, error) {
	...
	//fazer um select para pegar as respostas da goruntime
	...
	// Encapsula as chamadas no select na estrutura de repetição
	for i := 0; i < 4; i++ {
		select {
...
// 4 funções que serão responsáveis de ir na API e buscar as informações
// BuscarDadosDoUsuario chama a API para buscar os dados base do usuário
func BuscarDadosDoUsuario(canal chan<- Usuario, usuarioID uint64, r *http.Request) { //chan<-    específica que será um canal que só recebe valores
...
func BuscarSeguidores(canal chan<- []Usuario, usuarioID uint64, r *http.Request) {
	...
	response, erro := requisicoes.FazerRequisicaoComAutenticacao(r, http.MethodGet, url, nil) //nil - significa que não terá corpo na requisiçãos
	if erro != nil {
		canal <- nil
		return
	}
	...
	if seguidores == nil {
		canal <- make([]Usuario, 0) //aqui eu passo um slice vazio, e não como nil. Isso não vai quebrar a aplicação
		return
	}
...
func BuscarPublicacoes(canal chan<- []Publicacao, usuarioID uint64, r *http.Request) {
	...
	if publicacoes == nil {
		canal <- make([]Publicacao, 0) //aqui eu passo um slice vazio, e não como nil. Isso não vai quebrar a aplicação
		return
	}
